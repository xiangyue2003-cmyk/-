...
import { X, Upload, Music, User, Volume2, Power, Image as ImageIcon, MousePointer, Bell, CheckCircle, Ghost, AlertTriangle, BookOpen, Skull, Zap, Ear } from 'lucide-react';
...
type Tab = 'identity' | 'cast' | 'bgm' | 'sfx' | 'scenery' | 'manual';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in p-4 md:p-10">
      <div className="w-full max-w-5xl h-[80vh] bg-[#0a0f14] border border-cyber-blue/30 flex flex-col md:flex-row shadow-[0_0_50px_rgba(0,243,255,0.1)] overflow-hidden rounded-sm">
        ...
        <div className="w-full md:w-64 bg-cyber-dark border-r border-white/10 flex flex-col">
          ...
          <nav className="flex-1 overflow-y-auto">
            ...
            <button 
              ...
              className={`w-full text-left px-6 py-4 border-l-2 transition-all flex items-center gap-3 ${activeTab === 'manual' ? 'bg-white/5 border-white text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
            >
              <BookOpen size={16} />
              <div>
                <div className="font-sans text-sm font-bold">生存手册</div>
                <div className="text-[10px] font-mono">SURVIVAL GUIDE</div>
              </div>
            </button>
        ...
        <div className="flex-1 bg-[#050505]/50 overflow-y-auto p-8 relative">
          ...
          {/* TAB: MANUAL (SURVIVAL GUIDE) */}
          {activeTab === 'manual' && (
             <div className="space-y-8 animate-fade-in pb-10">
                <div className="border-b border-gray-800 pb-4 mb-4">
                   <h3 className="text-3xl font-display text-white mb-1">生存手册</h3>
                   <div className="text-xs font-mono text-gray-500 tracking-[0.3em] uppercase">The Key to Survival</div>
                </div>

                {/* Core Concept */}
                <div className="grid grid-cols-1 gap-6">
                   <div className="bg-gradient-to-br from-cyber-blue/10 to-transparent p-6 border-l-4 border-cyber-blue">
                      <div className="flex items-center gap-3 mb-3 text-cyber-blue">
                         <Ear size={20} />
                         <h4 className="font-bold text-lg">核心机制：静默与噪音</h4>
                      </div>
                      <p className="text-sm text-gray-300 leading-relaxed">
                         <strong className="text-white">噪音 (Noise)</strong> 是你最大的敌人，也是你唯一的武器。
                         <br/><br/>
                         在游戏早期（第一、二幕），你需要通过<span className="text-cyber-blue">「长按按钮」</span>来执行<strong className="text-white">静默行动 (Stealth)</strong>。这会降低噪音值，避免引起“教母”的注意。
                         <br/><br/>
                         如果噪音值超过 <span className="text-cyber-red font-mono">80%</span>，教母将实体化并试图抹杀你。
                      </p>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                   <div className="bg-white/5 p-4 rounded border border-white/5">
                      <div className="flex items-center gap-2 text-cyber-green mb-2">
                         <Zap size={16} />
                         <span className="font-bold text-sm">同步率 (Health)</span>
                      </div>
                      <p className="text-xs text-gray-400">代表你的生命体征。归零即死亡。</p>
                   </div>
                   <div className="bg-white/5 p-4 rounded border border-white/5">
                      <div className="flex items-center gap-2 text-cyber-purple mb-2">
                         <Ghost size={16} />
                         <span className="font-bold text-sm">清醒度 (Sanity)</span>
                      </div>
                      <p className="text-xs text-gray-400">精神状态。过低会导致幻觉和文字乱码。</p>
                   </div>
                   <div className="bg-white/5 p-4 rounded border border-white/5">
                      <div className="flex items-center gap-2 text-cyber-red mb-2">
                         <Skull size={16} />
                         <span className="font-bold text-sm">教母腐化度 (Boss HP)</span>
                      </div>
                      <p className="text-xs text-gray-400">只有在第三幕后才能对其造成有效伤害。</p>
                   </div>
                </div>

                {/* Act Guides */}
                <div className="space-y-4 mt-4">
                   <h4 className="text-sm font-mono text-gray-500 uppercase tracking-widest border-b border-gray-800 pb-2">阶段攻略 (Phase Guide)</h4>
                   
                   <div className="flex gap-4 items-start group">
                      <div className="font-display text-2xl text-gray-700 font-bold group-hover:text-cyber-blue transition-colors">01</div>
                      <div>
                         <div className="font-bold text-gray-200">糖果囚笼</div>
                         <p className="text-xs text-gray-500 mt-1">无论发生什么，保持安静。不要吃任何东西。寻找逃跑路线。</p>
                      </div>
                   </div>

                   <div className="flex gap-4 items-start group">
                      <div className="font-display text-2xl text-gray-700 font-bold group-hover:text-cyber-blue transition-colors">02</div>
                      <div>
                         <div className="font-bold text-gray-200">致命午睡</div>
                         <p className="text-xs text-gray-500 mt-1">视线会变得模糊。利用“静默”行动来维持理智。教母在听。</p>
                      </div>
                   </div>

                   <div className="flex gap-4 items-start group">
                      <div className="font-display text-2xl text-gray-700 font-bold group-hover:text-cyber-red transition-colors">03</div>
                      <div>
                         <div className="font-bold text-cyber-red/80">噪音反噬 (关键转折)</div>
                         <p className="text-xs text-gray-500 mt-1">规则改变。当你获得特定道具（如录音机、扩音器）后，<strong className="text-gray-300">故意制造噪音</strong>可以伤害教母。是时候反击了。</p>
                      </div>
                   </div>
                </div>
             </div>
